...
class ConfigCliente:
    ...
    def codagente_fs(self) -> str | None:
        """Codigo del agente FS asignado a esta empresa."""
        return self.empresa.get("codagente_fs")

    @property
    def fs_url(self) -> str | None:
        """URL base de la instancia FS de esta empresa."""
        return self.empresa.get("fs_url")

    @property
    def fs_token(self) -> str | None:
        """Token API de la instancia FS de esta empresa."""
    ...
    def buscar_por_cif(self, cif: str) -> Optional[tuple]:
        ...
        Returns:
            (clave, datos, rol) o None. rol = "proveedor" o "cliente"
        """
        cif_norm = _normalizar_cif(cif)
        if not cif_norm:
            return None
