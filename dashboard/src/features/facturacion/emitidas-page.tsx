...
type FiltroEstado = 'todas' | 'cobradas' | 'pendientes'
...
export default function EmitidasPage() {
  ...
  const facturasFiltradas = useMemo(() => {
    ...
    if (filtroEstado === 'cobradas') resultado = resultado.filter((f) => f.pagada)
  ...
  const cobradas = useMemo(() => facturas.filter((f) => f.pagada), [facturas])
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KPICard
          ...
          descripcion={`${formatearImporte(cobradas.reduce((a, f) => a + (f.total ?? 0), 0))}`}
          icono={CheckCircle}
          cargando={isLoading}
        />
        <KPICard
          titulo="Pendiente cobro"
          valor={formatearImporte(pendientesCobro)}
          icono={Clock}
          cargando={isLoading}
          invertirColor
        />
      ...
      <DataTable
        ...
        vacio="No hay facturas emitidas"
      />
    </div>
  )
}
